...
class StatesModule {
	...
	private recreate(): { warning?: string; error?: string; states?: State[] } {
		...
		const statesCount =
			ensureEl<HTMLInputElement>("statesNumber").valueAsNumber;
		if (!statesCount)
			return {
				error:
					"<i>States Number</i> option value is zero. No counties are generated",
			};
		...
		if (!validBurgs.length)
			return {
				error:
					"There are no burgs to generate states. Please create burgs first",
			};
		...
		const sortedBurgs = validBurgs
			...
			.map((burg): [typeof burg, number] => [
				burg,
				(burg.population ?? 0) * Math.random(),
			])
			.sort((a, b) => b[1] - a[1])
			.map(([burg]) => burg);
		...
		const newStates: State[] = [
			{ ...pack.states[0], i: 0, name: pack.states[0].name },
		];
		...
		for (const cellId of pack.cells.i) {
			pack.cells.state[cellId] =
				lockedStateIds.indexOf(pack.cells.state[cellId]) + 1;
		}
		...
		for (let stateId = newStates.length; stateId < count; stateId++) {
			...
			const expansionism = rn(
				Math.random() *
					ensureEl<HTMLInputElement>("sizeVariety").valueAsNumber +
					1,
				1,
			);
	...
	private createStates() {
		const states: State[] = [
			{ i: 0, name: "Neutrals", salesTax: 0, pollTax: 0, treasury: 0 } as State,
		];
	...
	private getBiomeCost(b: number, biome: number, type: string) {
		...
		if (type === "Nomadic" && biome > 4 && biome < 10)
			return pack.biomes[biome].cost * 3; // forest biome penalty for nomads
	...
	private getHeightCost(h: number, i: number, type: string) {
		const isHabitableWater =
			h < 20 && pack.biomes[pack.cells.biome[i]].habitability > 0;
		if (isHabitableWater) {
	...
	expandStates() {
		...
		while (queue.length) {
			...
			cells.c[e].forEach((e) => {
				...
				const isHabitableWater =
					cells.h[e] < 20 && pack.biomes[cells.biome[e]].habitability > 0;
				...
				if (!cost[e] || totalCost < cost[e]) {
					const isHabitableWater =
						cells.h[e] < 20 && pack.biomes[cells.biome[e]].habitability > 0;
					if (cells.h[e] >= 20 || type === "Aquatic" || isHabitableWater)
	...
	normalize() {
		...
		for (const i of cells.i) {
			const isHabitableWater =
				cells.h[i] < 20 && pack.biomes[cells.biome[i]].habitability > 0;
			if (
				(cells.h[i] < 20 &&
					pack.states[cells.state[i]]?.type !== "Aquatic" &&
					!isHabitableWater) ||
				cells.burg[i]
			)
	...
	generateCampaign(state: State): Campaign[] {
		...
		return neighbors
			.map((i: number) => {
				...
				return {
					name: `${getAdjective(name)} ${rw(wars)}`,
					start,
					end,
					attacker: state.i!,
					defender: i,
		...
		for (let f = 1; f < states.length; f++) {
			...
			for (let t = f + 1; t < states.length; t++) {
				...
				const neibOfNeib =
					naval || neib
						? false
						: states[f].neighbors!.some((n) =>
								states[n].neighbors!.includes(t),
							);
	...
	defineTaxRates(state: State) {
		const { salesTax, pollTax } =
			DEFAULT_TAX_BY_FORM[state.form || ""] || DEFAULT_TAX;
		return {
			salesTax: rn(
				gauss(salesTax, salesTax * 0.15, salesTax * 0.5, salesTax * 1.5, 4),
				2,
			),
			pollTax: rn(
				gauss(pollTax, pollTax * 0.15, pollTax * 0.5, pollTax * 1.5, 4),
				2,
			),
		};
	}
