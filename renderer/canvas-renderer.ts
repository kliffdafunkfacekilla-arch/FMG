...
function blendColor(hex: string, rgbStr: string, weight: number): string {
	let r1 = 0,
		g1 = 0,
		b1 = 0;
	if (hex.startsWith("#")) {
		if (hex.length === 4) {
			r1 = parseInt(hex[1] + hex[1], 16);
			g1 = parseInt(hex[2] + hex[2], 16);
			b1 = parseInt(hex[3] + hex[3], 16);
		} else if (hex.length === 7) {
			r1 = parseInt(hex.substring(1, 3), 16);
			g1 = parseInt(hex.substring(3, 5), 16);
			b1 = parseInt(hex.substring(5, 7), 16);
		}
	}
	let r2 = 20,
		g2 = 40,
		...
		for (let i = 0; i < pointsN; i++) {
			...
			if (layerType === "heightmap" && heights) {
				...
				if (cultId > 0) {
					...
					color =
						heights[i] < 20
							? blendColor(baseColor, "rgb(20,40,200)", 0.5)
							: baseColor;
				} else {
					color = getHeightColor(heights[i]);
				}
				...
				if (stateId > 0) {
					const baseColor = STATE_COLORS[(stateId - 1) % STATE_COLORS.length];
					color =
						heights[i] < 20
							? blendColor(baseColor, "rgb(20,40,200)", 0.5)
							: baseColor;
				...
				if (goodId > 0) {
					const baseColor = GOODS[goodId].color;
					color =
						heights[i] < 20
							? blendColor(baseColor, "rgb(20,40,200)", 0.5)
							: baseColor;
