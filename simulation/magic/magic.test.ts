...
describe("Custom Magic System Simulation", () => {
  it("should generate magic nodes, ley-lines, intensity flux, and mages", () => {
    ...
    const magicTypes: MagicTypeConfig[] = [
      {
        ...
        weights: { production: 1.2, military: 1.5, ecology: 0.8, growth: 1.0, taxation: 1.0 }
      }
    ];
    ...
    const states = [
      { id: 1, name: "State A", color: "#111", capital: 1, center: 25, treasury: 1000, militaryPower: 100 }
    ];
    const cellStates = new Uint8Array(pointsN).fill(1);
    
    applyMagicGeopoliticalVectors(states, [], cellStates, flux, mages, magicTypes);
    ...
    const volatileTypes: MagicTypeConfig[] = [
      {
        ...
        weights: { production: 1.0, military: 1.0, ecology: 1.0, growth: 1.0, taxation: 1.0 }
      }
    ];
