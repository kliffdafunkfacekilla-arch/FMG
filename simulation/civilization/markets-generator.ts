  ...
  for (const b of burgs) {
    ...
    for (const gIdStr of Object.keys(GOODS)) {
      ...
      if (good.demandCoverage) {
        if (good.demandCoverage.food) itemDemand += foodTarget * good.demandCoverage.food;
        if (good.demandCoverage.utilities) itemDemand += utilitiesTarget * good.demandCoverage.utilities;
        if (good.demandCoverage.construction) itemDemand += constructionTarget * good.demandCoverage.construction;
        if (good.demandCoverage.military) itemDemand += militaryTarget * good.demandCoverage.military;
        if (good.demandCoverage.luxury) itemDemand += luxuryTarget * good.demandCoverage.luxury;
      }
