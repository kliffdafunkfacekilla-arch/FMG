	...
	getMethods(cellId: number = this.cellId) {
		return {
			...
			minHabitability: (min: number) =>
				biomesData.habitability[pack.cells.biome[cellId]] >= min,
			habitability: () =>
				biomesData.habitability[this.cells.biome[cellId]] > currentRNG() * 100,
...
export const GOODS: Record<
	number,
	{ id: number; name: string; color: string; value: number }
> = {};
