...
export function generateClimate(
	...
	if (options.seasonOffset !== undefined && options.axialTilt !== undefined) {
		const latShift = options.seasonOffset * options.axialTilt;
		latN += latShift;
	}
	...
	const cellsNumberModifier = (grid.cellsDesired / 10000) ** 0.25;
	...
	for (let i = 0; i < grid.cellsY; i++) {
		...
		const latBand = minmax(
			Math.floor((Math.abs(lat) - 1) / 5),
			0,
			...
			for (let s = 0; s < steps; s++) {
				...
				if (heights[current] < 20) {
					...
					const isPassable =
						nextCell >= 0 &&
						nextCell < pointsN &&
						heights[nextCell] <= MAX_PASSABLE_ELEVATION;
					...
					if (isPassable && nextCell < pointsN) {
						...
						const mod = (heights[nextCell] / 70) ** 2;
