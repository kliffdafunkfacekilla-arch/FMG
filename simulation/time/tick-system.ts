import { CustomMonth, CustomSeason, CustomMoon } from "../../state/store";
...
export class TickSystem {
  ...
  constructor(cycles?: Partial<PlanetaryCycles>, initialState?: CalendarState) {
    ...
    const legacyDaysPerMonth = cycles ? (cycles as any).daysPerMonth : undefined;
    const legacyMonthsPerYear = cycles ? (cycles as any).monthsPerYear : undefined;
    const daysPerWeek = cycles?.weekdays?.length ?? defaultCycles.weekdays.length;
    ...
    const safeCycles: PlanetaryCycles = {
      ...
      months: cycles?.months ?? (legacyDaysPerMonth && legacyMonthsPerYear
  ...
  public advance(ticks: number = 1): CalendarState {
    ...
    this.state.moonPhases = moons.map(moon => {
      ...
      const totalRatios = moon.customPhases.reduce((sum, p) => sum + p.ratio, 0) || 1.0;
    ...
    const moonModProduct = this.state.moonPhases.reduce((prod, m) => prod * m.modifier, 1.0);
    const prodMod = (activeSeason ? activeSeason.prodMod : 1.0) * moonModProduct;
