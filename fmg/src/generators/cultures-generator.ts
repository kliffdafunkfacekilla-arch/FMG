	...
	generate() {
		...
		const defineCultureType = (i: number) => {
			...
			const isHabitableWater =
				this.cells.h[i] < 20 &&
				pack.biomes[this.cells.biome[i]].habitability > 0;
			if (isHabitableWater) return "Aquatic";
			const types: CultureType[] = [
				"Generic",
				"Hunting",
				"Highland",
				"River",
	...
	expand() {
		...
		const getBiomeCost = (c: number, biome: number, type: string) => {
			...
			if (type === "Nomadic" && biome > 4 && biome < 10)
				return pack.biomes[biome].cost * 10; // forest biome penalty for nomads
		...
		const getHeightCost = (i: number, h: number, type: string) => {
			...
			const isHabitableWater =
				h < 20 && pack.biomes[cells.biome[i]].habitability > 0;
			if (isHabitableWater) {
	...
	regenerate(): void {
		...
		pack.states = pack.states.map((state) =>
			!state.i || state.removed
				? state
				: { ...state, culture: pack.cells.culture[state.center] },
		);
		pack.burgs = pack.burgs.map((burg) =>
			!burg.i || burg.removed
				? burg
				: { ...burg, culture: pack.cells.culture[burg.cell] },
		);
		pack.religions = pack.religions.map((religion) =>
			!religion.i || religion.removed
				? religion
				: { ...religion, culture: pack.cells.culture[religion.center] },
		);
	}
}
