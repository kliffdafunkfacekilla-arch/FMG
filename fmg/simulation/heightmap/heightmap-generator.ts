import { createPRNG, type PRNG } from "../../core/random";
...
export class HeightmapGenerator {
		...
		for (let i = 0; i < count; i++) {
			...
			while (queue.length) {
				...
				for (const c of this.grid.cells.c[q]) {
					...
					change[c] = change[q] ** this.blobPower * (this.rng() * 0.2 + 0.9);
	...
	addPit(
		...
		for (let i = 0; i < count; i++) {
			...
			while (queue.length) {
				const q = queue.shift() as number;
				h = h ** this.blobPower * (this.rng() * 0.2 + 0.9);
				if (h < 1) break;
	...
	addRange(
		...
		for (let k = 0; k < count; k++) {
			...
			const startCellId = findGridCell(
				startX,
				startY,
				this.grid,
				this.width,
			...
			const getRidgeRange = (cur: number, end: number) => {
				...
				while (cur !== end) {
					...
					for (const e of this.grid.cells.c[cur]) {
						...
						let diff =
							(this.grid.points[end][0] - this.grid.points[e][0]) ** 2 +
							(this.grid.points[end][1] - this.grid.points[e][1]) ** 2;
			...
			while (queue.length) {
				...
				for (const idx of frontier) {
					this.heights[idx] = lim(
						this.heights[idx] + h * (this.rng() * 0.3 + 0.85),
					);
				}
			...
			ridge.forEach((cur: number, d: number) => {
				...
				for (let l = 0; l < stepCount; l++) {
					...
					this.heights[downhill] = Math.round(
						(this.heights[cur] * 2 + this.heights[downhill]) / 3,
					);
					cur = downhill;
				}
			});
	...
	addStrait(widthStr: string, direction = "vertical"): void {
		const desiredWidth = Math.min(
			getNumberInRange(widthStr, this.rng),
			this.grid.cellsX / 3,
		);
		...
		const startX = vert
			? Math.floor(this.rng() * this.width * 0.4 + this.width * 0.3)
			: 5;
		const startY = vert
			? 5
			: Math.floor(this.rng() * this.height * 0.4 + this.height * 0.3);
		const endX = vert
			? Math.floor(
					this.width -
						startX -
						this.width * 0.1 +
						this.rng() * this.width * 0.2,
				)
			: this.width - 5;
		const endY = vert
			? this.height - 5
			: Math.floor(
					this.height -
						startY -
						this.height * 0.1 +
						this.rng() * this.height * 0.2,
				);

		const start = findGridCell(
			startX,
			startY,
			this.grid,
			this.width,
		...
		for (let i = 0; i < desiredWidth; i++) {
			...
			range.forEach((r: number) => {
				for (const e of this.grid.cells.c[r]) {
					...
					used[e] = 1;
					query.push(e);
					this.heights[e] = lim(this.heights[e] ** exp);
